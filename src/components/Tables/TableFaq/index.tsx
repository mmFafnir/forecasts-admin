...
import { fetchAllFaq } from "../../../store/Slices/faqSlice/ayncActions";
import UpdateFaq from "../../../modules/Forms/faq-from/UpdateFaq";
import { IFaq } from "../../../store/Slices/faqSlice/interface";

const TableFaq: FC = () => {
  const dispatch = useTypeDispatch();
  const { faqs, status } = useTypeSelector((state) => state.faq);

  const [isOpenModal, setIsOpenModal] = useState<boolean>(false);
  const [modalFaq, setModalFaq] = useState<IFaq | null>(null);

  const onModalOpen = (faq: IFaq) => {
    console.log(faq);
    setModalFaq(faq);
    setIsOpenModal(true);
  };
          ...
          columns={columns.map((item) => {
            if (item.key === "action") {
              item.render = (_, record) => (
                <button onClick={() => onModalOpen(record)}>
                  <svg
