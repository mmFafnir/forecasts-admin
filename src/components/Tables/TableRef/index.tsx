...
import { fetchRefs } from "../../../store/Slices/refsSlice/asyncActions";
import Pagination from "../../UI/Pagination";
import { UpdateRef } from "../../../modules/Forms/refs-form";
import { TypeRef } from "../../../store/Slices/refsSlice/interface";

const TableRefs: FC = () => {
  const { refs, status, total } = useTypeSelector((state) => state.refs);
  const { limit, search } = useTypeSelector((state) => state.filters);
  const dispatch = useTypeDispatch();

  const [page, setPage] = useState<number>(1);
  const [isOpenModal, setIsOpenModal] = useState<boolean>(false);
  const [ref, setRef] = useState<TypeRef | null>(null);

  const openModal = () => setIsOpenModal(true);
  const closeModal = () => setIsOpenModal(false);

  const onGetRefs = (params: Pick<TFilter, "page" | "search" | "limit">) => {
    dispatch(fetchRefs(params));
  };
