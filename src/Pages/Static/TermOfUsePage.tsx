...
const getTermOfUsePage = async () => {
  const { data } = await axios.get("/get_term_of_use");
  return data.data;
};

const updateTermOfUsePage = async (values: IFetchPolicy) => {
  const { data } = await axios.post("/update_term_of_use", values);
  console.log(data);
};
